...
def generateWhiteLightInterferogram(xMin, xMax, N):
    ...
    k2 = 1 / 0.8
    y = 1 + np.exp(-x * x / 4) * (
                np.sin(2 * np.pi * (k1 + k2) * x / 2) / x * np.sin(2 * np.pi * (k1 - k2) * x / 2) + noise)
    return (x, y)
...
if __name__ == '__main__':
    Directory: str = "Données_Michelson/"
    # Basse resolution
    # (x, y) = generateHeNeInterferogram(xMin=-10, xMax=10, N=200)  # en microns
    x, y = readVectorsFromFile(Directory+"laser_he_ne_3.txt")
    (w, f, s) = fourierTransformInterferogram(x, y)
    df = f[1] - f[0]
    dl = 0.6328 * 0.6328 * df * 1000  # x 1000 pour nm
    plotCombinedFigures(x, y, w, s, left=632.8 - 5 * dl, right=632.8 + 5 * dl,
                        title="Spectre He-Ne basse resolution {0:0.2f} nm".format(dl))
    ...
    plotCombinedFigures(x, y, w, s, left=632.8 - 5 * dl, right=632.8 + 5 * dl,
                        title="Spectre He-Ne haute resolution {0:0.2f} nm".format(dl))
    ...
    dl = 0.6328 * 0.6328 * df * 1000
    plotCombinedFigures(x, y, w, s, left=632.8 - 5 * dl, right=632.8 + 5 * dl,
                        title="Spectre He-Ne tres haute resolution {0:0.2f} nm".format(dl))
    ...
    dl = 0.6328 * 0.6328 * df * 1000
    plotCombinedFigures(x, y, w, s, left=632.8 - 5 * dl, right=632.8 + 5 * dl,
                        title="Spectre He-Ne hyper haute resolution {0:0.2f} nm".format(dl))
    ...
    x, y = readVectorsFromFile(Directory + "lum_blanche_6.txt")
    (w, f, s) = fourierTransformInterferogram(x, y)
    df = f[1] - f[0]
